...
export async function outer(__typeArgs: $.GenericTypeArgs | undefined, ch: $.Channel<any> | null): globalThis.Promise<any> {
	return (await inner({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: __typeArgs?.["T"] ?? { type: { kind: $.TypeKind.Interface, methods: [] }, zero: () => null }}, ch) as any)
}

export async function main(): globalThis.Promise<void> {
	let ch: $.Channel<number> | null = $.makeChannel<number>(1, 0, "both")
	await $.chanSend(ch, 7)
	await $.println("value:", await outer({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}, ch))
}
