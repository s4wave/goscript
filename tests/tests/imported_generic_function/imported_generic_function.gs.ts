...
export function collectValues(value: helper.Value | null): [$.Slice<helper.Value | null>, $.GoError] {
	const __goscriptReturn0 = helper.Collect({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: "helper.Value", zero: () => null, methods: {GetValue: (receiver: any, ...args: any[]) => receiver.GetValue(...$.stripGenericTypeArgs(args))} }}, value)
...
export function collectAssigned(value: helper.Value | null): [$.Slice<helper.Value | null>, $.GoError] {
	let __goscriptTuple0: any = helper.Collect({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: "helper.Value", zero: () => null, methods: {GetValue: (receiver: any, ...args: any[]) => receiver.GetValue(...$.stripGenericTypeArgs(args))} }}, value)
...
export async function main(): globalThis.Promise<void> {
	let box = ($.markAsStructValue($.cloneStructValue(helper.Wrap({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}, 21))) as helper.Box)
	await $.println("wrapped:", box.Value)
	...
	await $.println("collected:", await $.pointerValue<Exclude<helper.Value, null>>($.arrayIndex(values!, 0)).GetValue())
	...
	await $.println("assigned:", await $.pointerValue<Exclude<helper.Value, null>>($.arrayIndex(assigned!, 0)).GetValue())
}
