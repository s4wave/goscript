...
export async function main(): globalThis.Promise<void> {
	...
	await $.println("min:", min({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}, 8, 3), min({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int", typeName: "main.Score" }, zero: () => 0 }}, 9, 4), min({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "string" }, zero: () => "" }}, "go", "ts"))

	await $.println("=== Generic stack ===")
	let stack: $.VarRef<Stack> = $.varRef($.markAsStructValue(new Stack()))
	stack.value.Push({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}, 10)
	stack.value.Push({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}, 20)
	let __goscriptTuple0: any = stack.value.Pop({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }})
	let value = (__goscriptTuple0[0] as number)
	let ok = __goscriptTuple0[1]
	await $.println("pop:", value, ok, stack.value.Len({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}))
	let __goscriptTuple1: any = stack.value.Pop({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }})
	...
	Set_Add(seen, {[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "string" }, zero: () => "" }}, "wasm")
	await $.println("set:", Set_Has(seen, {[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "string" }, zero: () => "" }}, "go"), Set_Has(seen, {[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "string" }, zero: () => "" }}, "rust"), $.len(seen))
	...
	let mapper: Mapper | $.VarRef<Mapper> | null = (NewMapper({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, K: { type: { kind: $.TypeKind.Basic, name: "string" }, zero: () => "" }, V: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}) as Mapper | $.VarRef<Mapper> | null)
	Mapper.prototype.Put.call(mapper, {[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, K: { type: { kind: $.TypeKind.Basic, name: "string" }, zero: () => "" }, V: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}, "answer", 42)
	let __goscriptTuple2: any = Mapper.prototype.Get.call(mapper, {[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, K: { type: { kind: $.TypeKind.Basic, name: "string" }, zero: () => "" }, V: { type: { kind: $.TypeKind.Basic, name: "int" }, zero: () => 0 }}, "answer")
	...
	let swapped = ($.markAsStructValue($.cloneStructValue($.markAsStructValue($.cloneStructValue(pair)).Swap({[$.genericTypeArgsMarker]: $.genericTypeArgsBrand, T: { type: { kind: $.TypeKind.Basic, name: "string" }, zero: () => "" }}))) as Pair)
