...
export function hash6(u: bigint, h: number): number {
	...
	return $.uint($.uint($.uint64Shr((BigInt.asUintN(64, ($.uint64Shl(u, 16n)) * 227718039650203n)), ($.uint($.uint64And(($.uint($.uint64Sub(64n, h), 64)), 63n), 64))), 32), 32)
}
...
export function highAfterMask(v: bigint): bigint {
	return $.uint64Mul((v & 65535n), (2 ** 48))
}

export function combineHighLow(v: bigint, low: bigint): bigint {
	return ($.uint64Mul((v & 65535n), (2 ** 48))) | $.uint64($.uint(low, 16))
}
...
export function setHighBit(idx: bigint): boolean {
	...
	words![Number($.uint64Div(idx, 64n))] = words![Number($.uint64Div(idx, 64n))] | ($.uint64Shl(1n, ($.uint64Mod(idx, 64n))))
...
export async function main(): globalThis.Promise<void> {
	...
	$.println($.uint($.uint(highAfterMask(4660n) >> 48n, 32), 32))
	$.println($.uint($.uint(combineHighLow(4660n, 48879n) >> 48n, 32), 32))
	$.println($.uint($.uint(combineHighLow(4660n, 48879n) & 65535n, 32), 32))
