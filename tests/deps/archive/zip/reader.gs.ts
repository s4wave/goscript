...
export class Reader {
	...
	public async init(rdr: io.ReaderAt | null, size: bigint): globalThis.Promise<$.GoError> {
		...
		if (($.pointerValue<__goscript_struct.directoryEnd>(end).directorySize < $.uint64(size)) && (($.uint64Div(($.uint64Sub($.uint64(size), $.pointerValue<__goscript_struct.directoryEnd>(end).directorySize)), 30n)) >= $.pointerValue<__goscript_struct.directoryEnd>(end).directoryRecords)) {
		...
		switch (fieldTag) {
			...
			case 10:
			{
				...
				while ($.len((fieldBuf.value as readBuf)) >= 4) {
					...
					let nsecs = $.int64Mul(($.int64Div(1e9, 10000000)), ($.int64Mod(ts, 10000000n)))
...
export async function findDirectory64End(r: io.ReaderAt | null, directoryEndOffset: bigint): globalThis.Promise<[bigint, $.GoError]> {
	let locOffset = $.int64Sub(directoryEndOffset, 20n)
	if (locOffset < 0n) {
		return [-1n, null]
	}
