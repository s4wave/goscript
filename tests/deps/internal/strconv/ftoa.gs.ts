...
export function genericFtoa(dst: $.Slice<number>, val: number, fmt: number, prec: number, bitSize: number): $.Slice<number> {
	...
	let mant = $.uint64And(bits, ($.uint64Sub(($.uint64Shl(1n, $.pointerValue<floatInfo>(flt).mantbits)), 1n)))
	let denorm = false
...
export function roundShortest(d: __goscript_decimal.decimal | $.VarRef<__goscript_decimal.decimal> | null, mant: bigint, exp: number, flt: floatInfo | $.VarRef<floatInfo> | null): void {
	...
	__goscript_decimal.decimal.prototype.Assign.call(upper, $.uint64Add(($.uint64Mul(mant, 2n)), 1n))
	...
	if ((mant > ($.uint64Shl(1n, $.pointerValue<floatInfo>(flt).mantbits))) || (exp == minexp)) {
		mantlo = $.uint64Sub(mant, 1n)
		explo = exp
	} else {
		mantlo = $.uint64Sub(($.uint64Mul(mant, 2n)), 1n)
		explo = exp - 1
	}
	let lower: __goscript_decimal.decimal | $.VarRef<__goscript_decimal.decimal> | null = new __goscript_decimal.decimal()
	__goscript_decimal.decimal.prototype.Assign.call(lower, $.uint64Add(($.uint64Mul(mantlo, 2n)), 1n))
...
export function fmtX(dst: $.Slice<number>, prec: number, fmt: number, neg: boolean, mant: bigint, exp: number, flt: floatInfo | $.VarRef<floatInfo> | null): $.Slice<number> {
	...
	if ((prec >= 0) && (prec < 15)) {
		...
		if (($.uint64Or(extra, ($.uint64And(mant, 1n)))) > 576460752303423488n) {
			mant++
		}
	...
	dst = $.append(dst, $.uint(48, 8), $.uint(fmt, 8), $.uint(48 + $.uint($.uint64And(($.uint64Shr(mant, 60n)), 1n), 8), 8), $.byteSliceHint)
	...
	if ((prec < 0) && (mant != 0n)) {
		dst = $.append(dst, $.uint(46, 8), $.byteSliceHint)
		while (mant != 0n) {
			dst = $.append(dst, $.uint($.indexStringOrBytes(hex, Number($.uint64And(($.uint64Shr(mant, 60n)), 15n))), 8), $.byteSliceHint)
			mant = $.uint64Shl(mant, 4n)
		}
	} else {
		if (prec > 0) {
