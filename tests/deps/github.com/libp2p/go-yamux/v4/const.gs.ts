...
export async function header_String(h: header): globalThis.Promise<string> {
	return fmt.Sprintf("Vsn:%d Type:%d Flags:%d StreamID:%d Length:%d", $.basicInterfaceValue(header_Version(h), "uint8"), $.basicInterfaceValue(header_MsgType(h), "uint8"), $.basicInterfaceValue(header_Flags(h), "uint16"), $.basicInterfaceValue(header_StreamID(h), "uint32"), $.basicInterfaceValue(header_Length(h), "uint32"))
}
