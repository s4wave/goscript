...
export class WebSocket {
	...
	public async OnMessage(fn: ((m: MessageEvent) => void) | null): globalThis.Promise<(() => void) | null> {
		...
		return $.markAsStructValue($.cloneStructValue(c)).addEventListener("message", $.functionValue(async (e: js.Value): globalThis.Promise<void> => {
			...
			if ($.markAsStructValue($.cloneStructValue(arrayBuffer)).Type() == js.TypeString) {
				...
				data = $.interfaceValue(extractArrayBuffer($.markAsStructValue($.cloneStructValue(arrayBuffer))), "[]byte", { kind: $.TypeKind.Slice, elemType: { kind: $.TypeKind.Basic, name: "uint8" } })
			}
	...
	public async SendBytes(v: $.Slice<number>): globalThis.Promise<$.GoError> {
		...
		try {
			__defer.defer(async () => { await handleJSError(err, (null as (() => void) | null)) })
			$.markAsStructValue($.cloneStructValue(c.v)).Call("send", $.interfaceValue($.markAsStructValue($.cloneStructValue(uint8Array(v))), "js.Value", "js.Value"))
	...
	public async addEventListener(eventType: string, fn: ((e: js.Value) => void) | null): globalThis.Promise<(() => void) | null> {
		...
		$.markAsStructValue($.cloneStructValue(c.v)).Call("addEventListener", eventType, $.interfaceValue($.markAsStructValue($.cloneStructValue(f)), "js.Func", "js.Func"))

		return $.functionValue((): void => {
			$.markAsStructValue($.cloneStructValue(c.v)).Call("removeEventListener", eventType, $.interfaceValue($.markAsStructValue($.cloneStructValue(f)), "js.Func", "js.Func"))
...
export function extractArrayBuffer(arrayBuffer: js.Value): $.Slice<number> {
	let uint8Array = $.markAsStructValue($.cloneStructValue($.markAsStructValue($.cloneStructValue($.markAsStructValue($.cloneStructValue(js.Global())).Get("Uint8Array"))).New($.interfaceValue($.markAsStructValue($.cloneStructValue(arrayBuffer)), "js.Value", "js.Value"))))
