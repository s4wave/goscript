...
> **OUTDATED / NON-AUTHORITATIVE.** This document has drifted from the actual
> compiler and runtime behavior and is not a source of truth. Where it conflicts
> with the compiler source, the `gs/` runtime overrides, or the passing
> compliance tests under `tests/tests/`, those win. It needs a full review and
> update. The numeric-types section below has been partially corrected to match
> the current integer representation; other sections may still be stale.
...
## Explicit Deferred Package Initialization

The opt-in `DeferredFunctions` compiler configuration replaces selected function imports with asynchronous ES module imports. Unlike Go's eager package initialization, a package reachable only through these boundaries initializes on first call. Taking a function value does not initialize it. Concurrent callers share module evaluation, and evaluation failures remain rejected on later calls. Static dependencies within the feature initialize through normal ES module ordering. Unconfigured builds keep their existing eager behavior. See `compiler/deferred-functions_test.go` for executable initialization and failure contracts.
    ...
    - **Identifiers:**
        ...
        - **Divergence/Considerations:**
            ...
            - **Predeclared Identifiers:** Go has several predeclared identifiers (e.g., `nil`, `iota`, `true`, `false`, built-in types like `int`, `string`, and functions like `make`, `len`, `append`, `println`). GoScript must map these:
                - `nil`: Typically maps to `null` or a custom `null` for type compatibility.
...
- **Divergence:** Major. TypeScript has no native support for imaginary literals. `3i` in TypeScript would be a compile-time error (if `i` is not defined) or interpreted as `3 * i`. GoScript needs a runtime representation for complex numbers and must translate imaginary literals into operations involving this representation (e.g., `$.complex(0, 3)`).
...
- **Divergence/Considerations:**
    ...
    - **Raw Strings:**
        ...
        - The current `WriteBasicLit` writes `exp.Value` directly. For raw strings, this means `c.tsw.WriteLiterally(\`raw string content\`)`. This is the problematic behavior.
...
-   **GoScript & Divergences:**
    ...
    -   **Complex Constants:**
        ...
        -   **Divergence:** GoScript represents complex numbers using a runtime type (e.g., `$.Complex` from `@goscript/builtin`). Complex constants must be translated into instantiations of this runtime type (e.g., `$.complex(realPart, imagPart)`). `compiler.WriteValueExpr` would need to handle this for constant expressions.
...
-   **GoScript & Divergences:**
    ...
    -   **`new(T)`:**
        ...
        -   **TypeScript:** No direct equivalent.
        -   **Divergence:** GoScript translates `new(T)` into code that creates an instance of the TypeScript type corresponding to `T`, ensuring it's properly zero-initialized, and then returns a "pointer" to it (often a varrefed value or a direct reference if `T` is a class). See `new T()` in runtime.
    -   **Pointers and Addressability:**
        ...
        -   **Divergence:** This is a fundamental difference. GoScript simulates pointers using:
            -   VarRefing: Wrapping primitive types or structs in a container object (e.g., `$.VarRef<T>`) to allow shared references and `nil` representation.
...
-   **GoScript & Divergences:**
    -   **General Mapping:** The core of GoScript is mapping Go types to TypeScript types.
        ...
        -   **Array Types:** `[N]T` -> `$.Array<T>` (custom runtime class to preserve value semantics and fixed size) or potentially `T[]` if value semantics are not strictly needed or handled differently. `compiler.WriteArrayType` uses `$.Array`.
        -   **Slice Types:** `[]T` -> `$.Slice<T>` (custom runtime class). `compiler.WriteSliceType`.
        -   **Map Types:** `map[K]T` -> `$.Map<K, T>` (custom runtime class) or `Map<K, T>` if keys are suitable. `compiler.WriteMapType` uses `$.Map`.
        -   **Channel Types:** `chan T` -> `$.Channel<T>` (custom runtime class). `compiler.WriteChannelType`.
        -   **Function Types:** `func(...) ...` -> TypeScript function types `(...) => ...`. `compiler.WriteFuncType`.
        -   **Pointer Types:** `*T` -> `$.VarRef<T>` for varRefing, or the direct TypeScript reference type if `T` maps to a class. `compiler.WritePointerType`.
    -   **Type Names & Qualified Identifiers:**
        -   **Go:** `pkg.TypeName`.
        -   **TypeScript:** `import { TypeName } from 'pkg';` then `TypeName`.
        -   **Divergence:** GoScript manages imports and translates qualified identifiers to appropriate TypeScript module access or imported names. `SemanticModelOwner` records package/file facts and `LoweringOwner` qualifies identifiers before `TypeScriptEmitOwner` renders imports.
...
-   **GoScript & Divergences:**
    -   **Integer Mapping (current behavior):**
        -   `int8`...`int32`, `uint8`...`uint32`, the platform-width signed `int`, and named types over any of these (for example `reflect.Kind`, which is `uint`-based): Map to TypeScript `number`. `int` stays `number` so it remains directly usable as an array index, slice bound, and `len`/`cap`/`make` size without coercion, and small enum-like named `uint` types stay plain numbers for cheap comparison.
        -   `int64` and `uint64` (and named types whose underlying type is one of these): Map to TypeScript `bigint`. They are always `bigint`, even for small values, so identity, equality, JSON, and type-assertion semantics are uniform. The runtime (`gs/builtin/builtin.ts`) provides `int64`/`uint64` conversion helpers plus the `int64*`/`uint64*` arithmetic family (add, sub, mul, div, mod, and, or, xor, shl, shr), all returning `bigint` with two's-complement (`int64`) or modular (`uint64`) wraparound.
        -   The platform-width unsigned `uint` and `uintptr`: Map to the TypeScript `number` type, but are 64-bit-wide and so cannot always fit a JS number exactly. The runtime represents them as a number for values up to `Number.MAX_SAFE_INTEGER` and falls back to a runtime `bigint` (still typed `number`) for larger values, so full 64-bit width survives (for example `^uint(0)` == 2^64-1). This is safe because `uint`/`uintptr` arithmetic always routes through the `int64*`/`uint64*` helper family, which accepts `number | bigint`; the `$.uint` conversion at every result boundary applies this widening. Index, slice-bound, and length sites already coerce with `Number(...)`.
        -   Because `bigint` cannot index a JS array or be used as a length, the compiler coerces a bigint-backed value with `Number(...)` wherever it flows into an index, slice bound, string/byte offset, or `make`/channel-capacity size.
        -   **Divergence:** TypeScript `number` only holds integers exactly up to `Number.MAX_SAFE_INTEGER`, so the strictly `number`-mapped integer types (including platform `int`) do not enforce Go's fixed-size overflow and lose precision past 2^53. The `bigint`-mapped `int64`/`uint64` types enforce correct full-width overflow, and `uint`/`uintptr` preserve full width through the runtime number-or-bigint representation above.
    ...
    -   **Complex Mapping:**
        -   `complex64`, `complex128`: Map to a GoScript runtime type like `$.Complex` (e.g., `{ real: number, imag: number }`).
...
-   **GoScript & Divergences:**
    ...
    -   **Indexing and `len`:**
        ...
        -   **Divergence:** For strings containing multi-byte UTF-8 characters (which become multiple UTF-16 code units, possibly surrogate pairs), `len` and indexing will yield different results/values. GoScript's `$.len(s)` for strings should ideally return byte length if strict Go compatibility is needed for this, or this difference must be documented. Standard string iteration in Go (`for range s`) yields runes (Unicode code points), which GoScript aims to replicate.
...
-   **GoScript & Divergences:**
    -   **Mapping:** Go array `[N]T` maps to `$.Array<T>` (a custom runtime class in `@goscript/builtin`).
        -   `compiler.WriteArrayType` generates `$.Array<TranslatedElementType>`.
    -   **Value Semantics:** `$.Array<T>` is designed to emulate Go's value semantics (e.g., by cloning on assignment/passing or by being an immutable structure that produces new instances on modification if that path were taken, though cloning is more direct for Go's model).
    -   **Fixed Length & Type Identity:**
        -   `$.Array<T>` internally manages its fixed length, initialized based on `N`.
        -   **Divergence:** TypeScript's type system doesn't natively express fixed-length arrays as part of the type like `$.Array<T, N>`. While `$.Array<T>` is one generic type in TS, instances will behave as fixed-length. The distinction that `[3]int` and `[4]int` are different Go types is maintained by GoScript generating distinct instantiations or by relying on the Go type checker's information. The generated TypeScript might look like `$.Array<number>` for both, but their construction and internal length differ.
    -   **Recursive Type Restriction:** This is primarily enforced by the Go compiler frontend. GoScript translates valid Go code.
    -   **Indexing:** `$.Array<T>` provides methods for element access that correspond to Go's `a[i]`.
...
-   **GoScript & Divergences:**
    -   **Mapping:** Go slice `[]T` maps to `$.Slice<T>` (a custom runtime class in `@goscript/builtin`).
        -   `compiler.WriteSliceType` generates `$.Slice<TranslatedElementType>`.
    -   **`nil` Slices:** `$.Slice<T>` must represent `nil` (e.g., an internal null pointer or a specific state).
    -   **Length, Capacity, Underlying Array:** `$.Slice<T>` implements these concepts. It manages a reference to an underlying data store (which could be a `$.Array<T>` or a standard TypeScript array internally) and tracks its own length and capacity.
    -   **Sharing Storage:** The implementation of `$.Slice<T>` must ensure that operations like slicing one slice from another result in shared underlying data as per Go semantics.
    -   **`append`, `copy`:** These built-in Go functions for slices are implemented as methods or helper functions on/for `$.Slice<T>` in the runtime.
...
-   **GoScript & Divergences:**
    -   **Mapping:** Go `*T` is mapped by `compiler.WritePointerType`.
        -   If `T` maps to a TypeScript primitive or a Go struct (which becomes a TS class designed for value semantics), `*T` typically maps to `$.VarRef<TranslatedT>` from `@goscript/builtin`. This wrapper simulates indirection, shared mutability, and `nil`.
        -   If `T` maps to a TypeScript class that is already a reference type (e.g., for Go interface types, or Go slices/maps which map to `$.Slice`/`$.Map` runtime classes), `*T` might map directly to `TranslatedT` (which can be `null`) or a more specific pointer wrapper if needed to distinguish `*MyClass` from `MyClass` when `MyClass` itself can be `nil`.
    -   **`nil` Value:** `$.VarRef<T>` can be `null` (or have an internal `null` value) to represent Go's `nil` pointer. Standard TypeScript reference types can also be `null`.
    -   **Dereferencing (`*p`):** Translated to `p.value` for `$.VarRef` or direct access if `p` is a direct reference.
    -   **Address-of (`&v`):** Translated to `new $.VarRef(v)` for values needing varRefing, or taking the reference directly if `v` is already a reference type suitable for Go's pointer semantics in that context.
...
-   **GoScript & Divergences:**
    ...
    -   **Runtime Behavior (Type Assertions/Switches):**
        ...
        -   **Divergence:** This is a major area. GoScript implements runtime type information (`$.typeInfo` attached to values or `$.VarRef`) and helper functions (`$.typeAssert`, `$.typeSwitch`) to emulate Go's type assertions (`x.(T)`) and type switches. This is handled by `compiler.WriteTypeAssertExpr` and `compiler.WriteStmtTypeSwitch`.
...
-   **GoScript & Divergences:**
    -   **Mapping:** Go `map[K]V` is mapped by `compiler.WriteMapType` to `$.Map<tsK, tsV>` (a custom class in `@goscript/builtin`).
    -   **`nil` Value:** `$.Map` instances can be `null` to represent Go's `nil` map.
    -   **Key Comparability:**
        -   Go's strict rules for key comparability (especially for structs or arrays as keys) require careful handling. `$.Map` needs to use a robust internal mechanism for key hashing and equality that mirrors Go's semantics (e.g., for struct keys, all fields are compared; for array keys, elements are compared).
        -   TypeScript's built-in `Map` uses `SameValueZero` for key equality, which works for primitives and object references. For Go's value-based key semantics with complex types, `$.Map` must implement custom key management.
        -   **Divergence:** Significant. TypeScript's `Map` keying is reference-based for objects. `$.Map` must simulate Go's value-based keying, potentially by serializing or deeply comparing complex keys, which can have performance implications.
    -   **Operations:**
    ...
    -   **Iteration Order:** Go map iteration order is explicitly undefined. TypeScript `Map.prototype.forEach` and `for...of` iteration preserve insertion order. `$.Map` should ideally mimic Go's undefined order if strict adherence is required, or document this divergence if it uses TS `Map`'s ordering.
...
-   **GoScript & Divergences:**
    -   **Mapping:** Go `chan T` is mapped by `compiler.WriteChannelType` to `$.Channel<tsT>` (a custom class in `@goscript/builtin`).
    -   **Concurrency Model:**
        ...
        -   **Divergence:** This is the most significant divergence. GoScript cannot directly replicate Go's goroutine-based concurrency and channel semantics in standard browser/Node.js JavaScript. `$.Channel` would likely be implemented using `async/await` and promises to simulate blocking operations, but it operates within the JS event loop model.
    -   **`nil` Channel:** `$.Channel` instances can be `null`.
    -   **Buffering:** `$.Channel` must simulate buffered and unbuffered behavior.
    ...
    -   **Select Statement:** Go's `select` statement, which waits on multiple channel operations, is a complex construct. Translating it (`compiler.WriteStmtSelect`) requires a sophisticated mechanism in `$.Channel` or a helper (e.g., `$.selectStatement`) to manage multiple asynchronous operations and choose the first one ready, mimicking Go's selection rules (including randomization for multiple ready channels).
    -   **Directionality:** `chan<-` and `<-chan` are compile-time checks in Go. GoScript can enforce this at the TypeScript type level to some extent (e.g., by providing different interfaces or views of `$.Channel`).
    -   **Limitations:** True goroutine preemption and scheduling are beyond GoScript's scope. The behavior of `$.Channel` will be an approximation within the JavaScript concurrency model. This means that code relying on fine-grained goroutine interleaving or specific scheduler behaviors may not translate perfectly.
...
-   **GoScript & Divergences:**
    ...
    -   **Pointer, Array, Slice, Map, Channel Types:** Identity depends on their element/base types. This structural approach should map reasonably to TypeScript's type system if the constituent types are mapped correctly.
        -   `$.VarRef<T>`, `$.Slice<T>`, `$.Map<K, V>`, `$.Channel<T>` would be generic types. `$.VarRef<number>` and `$.VarRef<MyInt>` would be different if `number` and `MyInt` are different.
...
-   **GoScript & Divergences:**
    ...
    -   **Rule 4 (Channel Assignability):**
        ...
        -   GoScript's `$.Channel<T>` would need to handle this. If `$.ChannelSend<T>` and `$.ChannelRecv<T>` are distinct types from `$.Channel<T>`, TypeScript's assignability rules would apply. A bidirectional `$.Channel<T>` should be assignable to `$.ChannelSend<T>` and `$.ChannelRecv<T>` (if they are supertypes or structurally compatible).
        -   **Divergence:** Depends on the `$.Channel` design.
    -   **Rule 5 (`nil` Assignability):**
        ...
        -   **Divergence:** GoScript maps Go `nil` to TS `null`. The target TS type must be nullable (e.g., `$.VarRef<T> | null`, `$.Slice<T> | null`). This is generally manageable.
...
-   **GoScript & Divergences:**
    ...
    -   **Value vs. Pointer Receivers:**
        ...
        -   **GoScript (Current):** All methods (both value and pointer receiver) are generated on the TypeScript class corresponding to `T`.
            ...
            -   Calls on `$.VarRef<T>` instances: `ptr_t_instance.value.Method()`. GoScript's `compiler.WriteSelectorExpr` needs to handle the `.value` access automatically for `$.VarRef` types when calling methods.
            -   **Divergence (Addressability for Pointer Receivers):** Go's automatic taking of address for pointer receiver calls on addressable values (e.g., `v.PtrMethod()` where `v` is `T`) is a subtle point. In TypeScript, if `v_ts` is an instance of class `T_ts`, `v_ts.PtrMethod()` would just call the method. The "pointer" nature of the original Go receiver is not directly enforced by TypeScript at the call site in the same way unless `v_ts` itself is explicitly a `$.VarRef`. GoScript aims to make method calls work transparently where Go allows them.
...
-   **GoScript & Divergences:**
    -   **Mapping:**
        -   **Types:**
            ...
            -   `error` -> `$.GoError` (custom interface/class in `@goscript/builtin`, typically `interface Error { error: string }` or similar to `Error` built-in).
            -   `complex64`, `complex128` -> `$.Complex` (custom class in `@goscript/builtin`).
            -   `any` (Go 1.18+, alias for `interface{}`) -> `any` in TypeScript.
            -   `comparable` (Go 1.18+, interface constraint) -> No direct single keyword. Used in generic constraints. TypeScript's `==`, `!=` work on many types. For struct/array comparison, GoScript needs custom logic (e.g., `$.equals`).
        -   **Constants:**
        ...
        -   **Functions:** These are built-in functions in Go.
            -   Mapped to functions/methods in `@goscript/builtin`. E.g., `len(s)` -> `s.length` (for strings/arrays) or `s.len()` (for `$.Slice`, `$.Map`), `append(sl, ...)` -> `$.append(sl, ...)`, `make(...)` -> `$.makeSlice()`, `$.makeMap()`, `$.makeChannel()`, `new(T)` -> `new T()(T_constructor)` or `new T_ts()`, `panic(v)` -> `$.panic(v)`, `recover()` -> `$.recover()`, `print`, `println` -> `console.log` or `$.println/println`.
            -   `clear` (Go 1.21+ for maps, slices) -> `map.clear()`, `slice.clear()` (or equivalent operations).
            -   `max`, `min` (Go 1.21+ for ordered types) -> `Math.max`, `Math.min` or custom `Math.max/min` for broader type support.
    -   **Divergence:** The primary "divergence" is that these are not globally available keywords/functions in TS but are provided via the `gs` runtime module or direct TS equivalents. The GoScript compiler must ensure correct translation (e.g., `len(x)` to `x.length` or `$.len(x)` depending on `x`'s type).
...
-   **GoScript & Divergences:**
    ...
    -   **Generic Aliases:**
        ...
        -   TS: `type MyMap<K extends string | number | symbol, V> = Map<K, V>;` (or `$.Map<K,V>`)
...
-   **GoScript & Divergences:**
    ...
    -   **`comparable` Exception & Runtime Behavior:**
        ...
        -   **Divergence (Moderate/Major):** The potential runtime panic for `comparable` type parameters when using `==` with interface type arguments is a significant Go-specific behavior. To emulate this, GoScript must translate `a == b` (where `a` or `b` are of a type parameter constrained by `comparable`) to a runtime helper like `$.equals(a, b)`. This helper would need to replicate Go's comparison logic, including the panic behavior for incomparable dynamic types within interfaces.
...
-   **GoScript & Divergences:**
    ...
    -   **Array Literals:**
        ...
        -   TS: `$.newArray(3, number, [1, 2, 3])`. Fixed-size arrays often require runtime helpers. Omitted values are zero-initialized in Go (e.g., `[3]int{1}` is `[1,0,0]`). GoScript's `WriteCompositeLit` must ensure this padding with zero values.
    -   **Slice Literals:**
        -   Go: `[]int{1, 2, 3}`.
        -   TS: `$.makeSlice(number, [1, 2, 3])` or directly `[1, 2, 3]` if the target type is `$.Slice<number>`. Runtime helpers are common.
    -   **Map Literals:**
        -   Go: `map[string]int{"a": 1, "b": 2}`.
        -   TS: `new T()Map<string, number>([["a", 1], ["b", 2]])`. Runtime helper `new T()Map` is used.
    ...
    -   **Divergences (Minor to Moderate):**
        ...
        -   **Arrays:** TypeScript lacks true fixed-size arrays distinct from dynamic arrays in the same way Go does. GoScript uses runtime helpers (`$.Array`) to emulate Go's array semantics (fixed size, distinct type from slice).
        -   **Zero-value padding:** For array literals like `[5]int{1,2}`, GoScript must explicitly pad with `0,0,0`.
        -   **Instantiation:** GoScript generally uses runtime helper functions (`$.newArray`, `$.makeSlice`, `new T()Map`) or static `new` methods on generated classes for composite literals to ensure correct Go semantics (e.g., cloning, type information).
...
-   **GoScript & Divergences (Continued):**
    ...
    -   **Array/Slice Literal Rules (Indices):**
        -   `$.newArray` and `$.makeSlice` helpers take an array of values. Keyed elements like `[5]int{2:10}` (meaning `[0,0,10,0,0]`) require GoScript to construct the full array with zero padding before passing to the helper. E.g., `$.newArray(5, number, [0,0,10,0,0])`.
    -   **Address of Composite Literal:**
        ...
        -   `&[]int{}` vs `new([]int)`:
            -   `p1 := &[]int{}` -> `let p1: $.Slice<number> | null = $.makeSlice(number, []);` (an initialized, empty slice).
            -   `p2 := new([]int)` -> `let p2: $.Slice<number> | null = null;` (a nil slice).
    ...
    -   **Array Literal Length & `...`:**
        -   `[10]string{}` -> `$.newArray(10, string)`.
        -   `[...]string{"a", "b"}` -> `$.newArray(2, string, ["a", "b"])`. The Go compiler resolves `...` to a fixed length; GoScript receives this length.
    ...
    -   **Eliding Literal Types:**
        -   Go: `[][]int{{1,2}}`
        -   TS: `$.makeSlice($.SliceInt, [$.makeSlice(number, [1,2])])`. TypeScript usually requires explicit construction or type information for nested structures if runtime helpers are involved. Type inference might simplify some cases, but GoScript often generates explicit calls.
...
-   **GoScript & Divergences:**
    ...
    -   **Closures:**
        ...
        -   **Divergence (Potentially Major for mutable captures):**
            ...
            -   GoScript's `analysis.NeedsVarRefed` and related functions detect variables that need to be heap-allocated (var-refed) due to closure semantics. The generated TS code then uses `$.VarRef<T>` for these variables (e.g., `let x = new $.VarRef(0); ... closure uses x.value ...`). This is a significant transformation to ensure Go's closure semantics for mutable captured variables are preserved.
    -   **Overall:** Function literals map well to TS arrow functions or function expressions. The primary complexity is emulating Go's closure capture semantics for mutable variables, which GoScript handles via varRefing analysis and runtime `$.VarRef` helpers.
...
-   **GoScript & Divergences:**
    ...
    -   **`PrimaryExpr Slice` (e.g., `s[i:j]`):**
        ...
        -   Handled by `compiler.WriteSliceExpr`, typically using a runtime helper like `$.goSlice(s, i, j)`.
    -   **`PrimaryExpr TypeAssertion` (e.g., `x.(T)`):**
        -   Asserting the dynamic type of an interface.
        -   Handled by `compiler.WriteTypeAssertExpr`, often using a runtime helper like `$.typeAssert(x, T_info)`.
...
-   **GoScript & Divergences:**
    ...
    -   **Rule 5 (Nil pointer field access):**
        ...
        -   GoScript can insert `nullPanicIfNil(p)` before `p.x` to ensure Go-like panic behavior, or rely on TS/JS runtime errors (`TypeError: Cannot read properties of null`). The `nullPanicIfNil` is more explicit.
    -   **Rule 6 (Nil interface method call):**
        ...
        -   Similar to pointers, `nullPanicIfNil(i)` can be used.
...
-   **GoScript & Divergences (Continued):**
    -   **`(*T).Mv` case:**
        ...
        -   TS: `const fn = (pPtr: $.VarRef<Point>, factor: number): void => { const pVal = $.unref(pPtr); pVal.ScaleByVal(factor); }` or if `Point` is a class, it might involve creating a temporary value if `ScaleByVal` modifies `this` and Go semantics require the original pointed-to value not to change. However, since `ScaleByVal` has a value receiver, it operates on a copy. So, `$.unref(pPtr).ScaleByVal(factor)` is more direct if `ScaleByVal` is on the class `Point`.
...
-   **GoScript & Divergences:**
    ...
    -   **Receiver Evaluation and Storage:**
        -   **Value Receiver on Value Type (`t.Mv` where `t` is `T`):
            ...
            -   **Divergence/Implementation:** GoScript needs to ensure value semantics. If `T` is a struct type (mapped to a class), `myInstance.MyMethod.bind(myInstance)` binds the *reference*. To match Go, if `Mv` is a value-receiver method, a *clone* of `myInstance` should be bound if `myInstance` is a value type.
                `let receiver_copy = $.clone(myInstance); const f = receiver_copy.Mv.bind(receiver_copy);`
        ...
        -   **Value Receiver on Pointer Type (`pt.Mv` where `pt` is `*T`):
            ...
            -   TS: `const val_of_pt = $.clone($.unref(pt)); const f = (...args) => val_of_pt.Mv(...args);`. Requires dereferencing and cloning.
        -   **Pointer Receiver on Addressable Value Type (`t.Mp` where `t` is `T`):
            ...
            -   TS: This is tricky. If `t` is a class instance, `t` is already a reference. If `t` is a primitive that needs varRefing for addressability, `const varRef_t = $.varRef(t); const f = (...args) => varRef_t.value.Mp(...args);` (assuming `Mp` is on the varrefed value's type, which is complex). More likely, `const f = t.Mp.bind(t)` if `Mp` is correctly on `T`'s prototype and handles value/pointer semantics internally.
    ...
    -   `compiler.WriteSelectorExpr` needs to be aware of these nuances to generate the correct binding or wrapper function, potentially involving `$.clone` or `$.varRefValueReceiverMethod` helpers.
    ...
    -   The Go spec example `*t = 42` not affecting `f` and `g` implies that for `f := t.M` where `t` is `*T` and `M` has a `T` receiver, `*t` is read *at the time of `t.M`* and this value is what `M` operates on. If `M` had a `*T` receiver, then the pointer `t` itself would be bound.
        ...
        -   GoScript: `let t_ptr = new T()(number, 10); const receiver_val = $.unref(t_ptr); const f = () => MyTImpl.prototype.M.call(receiver_val); $.set(t_ptr, 42); f();`
...
-   **GoScript & Divergences (Continued):**
    -   **`pt.Mv` (Value Receiver, Pointer Instance):**
        ...
        -   TS: `const derefed_pt = $.clone($.unref(pt)); const f = (...args) => derefed_pt.Mv(...args);`
...
-   **GoScript & Divergences (`compiler.WriteSelectorExpr` for method values):**
    -   **Mapping to TS:**
        ...
        -   Example: `const pPtr = $.varRef(new Point(1,2)); const distFunc = pPtr.value.Distance.bind(pPtr.value);` (assuming `Distance` is on `Point` and `pPtr.value` is the `Point` instance).
...
-   **GoScript & Divergences:**
    ...
    -   **Arrays/Slices/Strings:**
        -   `a[x]` maps to `a.get(x)` or `$.index(a, x)` for runtime bounds checking and consistent panic behavior.
        -   String indexing `s[i]`: Go returns `byte`. TS `s.charCodeAt(i)` returns `number` (UTF-16 code unit).
            -   **Divergence:** `s.charCodeAt(i)` is not a byte. `new TextEncoder().encode(s[i])[0]` would be closer but inefficient. GoScript likely uses a helper `$.indexString(s, i)` returning `number` (byte value).
    ...
    -   **Maps:**
        ...
        -   `a[x] = val`: `a.set(x, val)`.
        -   `v, ok = a[x]`: `const [v, ok] = [a.get(x) ?? zeroValueForV, a.has(x)];` (or a helper `$.mapGet(a,x)` returning `[V, boolean]`). `compiler.writeChannelReceiveWithOk` (misnamed?) or similar logic is used for this pattern.
        -   Nil map panic: `$.mapSet(m, k, v)` and `$.mapGet(m, k)` helpers would check for `nil` map and panic.
...
-   **GoScript & Divergences:**
    ...
    -   **Mapping:** `a[low:high]` typically maps to `$.goSlice(a, low, high)`.
    ...
    -   **Underlying Array Sharing (for Go slices):**
        ...
        -   **Major Divergence:** To emulate Go's slice semantics (shared underlying array and capacity), GoScript's `$.Slice<T>` type and `$.goSlice()` helper are crucial. `$.Slice` would internally hold a reference to the underlying array/data source, an offset, length, and capacity.
...
-   **GoScript & Divergences (Continued):**
    -   **Simple Slice Defaults:** `$.goSlice(a, low, high)` helper needs to handle default `low` (0) and `high` (`$.len(a)`).
    -   **Pointer to Array Slicing:** `WriteSliceExpr` should handle dereferencing: `$.goSlice($.unref(a), low, high)`.
    -   **Index Range Checks:** `$.goSlice` must perform runtime checks and panic as per Go rules.
        -   `cap(a)` for slices: `$.Slice<T>` needs a `capacity` property and `$.cap(a)` helper.
    -   **Addressable Array for Slicing:** Go requires array to be addressable to take a slice from it.
        -   TS arrays are reference types. If GoScript maps Go arrays to TS arrays/`$.Array`, this is generally fine.
    ...
    -   **Nil Slice Slicing:** `$.goSlice(nilSlice, ...)` should return `null` (or GoScript's `nil` slice representation).
    -   **Underlying Array Sharing & Capacity (Full Slice):**
        -   This is the core challenge for `$.Slice<T>` and `$.goSlice(a, low, high, max?)`.
        -   `$.goSlice(a, low, high)`: capacity is `$.cap(a) - low`.
        -   `$.goSlice(a, low, high, max)`: capacity is `max - low`.
        -   The `$.Slice` object must contain:
            -   Reference to the original data source (e.g., a TS array or another `$.Slice`'s underlying data).
            -   Offset into that data source.
            -   Length.
            -   Capacity.
        -   Modifications to elements of a `$.Slice` must affect the original data source if it's shared.
        -   Example: `t := a[1:3:5]` (where `a` is `[5]int{1,2,3,4,5}`)
            -   `t` length 2 (`t[0]=2, t[1]=3`), capacity 4 (`5-1`).
            -   `$.goSlice(a, 1, 3, 5)` would create a `$.Slice` pointing to `a`'s data, offset 1, length 2, capacity 4.
...
-   **GoScript & Divergences:**
    ...
    -   **Mapping `x.(T)`:**
        -   `$.typeAssert(x, T_descriptor)`: This runtime helper would check the dynamic type of `x` against `T_descriptor`.
        -   `T_descriptor` would be a runtime representation of type `T`, including its methods if it's an interface.
        -   If `x` is `null` or type doesn't match, `$.typeAssert` panics.
        -   Otherwise, it returns `x` (cast to `T` in TS).
    -   **Mapping `v, ok = x.(T)`:**
        -   `[v, ok] = $.typeAssertWithOk(x, T_descriptor)`
        -   `$.typeAssertWithOk` returns `[value, boolean]`. If assertion fails, returns `[zeroValueForT, false]`.
...
-   **GoScript & Divergences (Continued):**
    ...
    -   **Nil Function Panic:** `f(...)` in TS: if `f` is `null` or `undefined`, TS throws a `TypeError`. This is similar to Go's panic. GoScript can rely on this or wrap calls in a helper for consistent error messages: `$.call(f, ...args)`.
    -   **Special Case `f(g())`:**
        ...
        -   **Divergence:** GoScript must transform `f(g())`. If `g` returns `[val1, val2]`, then `f(...$.multiRet(g()))` or `const ret_g = g(); f(ret_g[0], ret_g[1])`. The `compiler.WriteCallExpr` needs to detect this pattern.
        -   If `g` returns multiple values and `f` is variadic: `f(...g())` in Go. TS: `f(...$.multiRet(g()))`.
    -   **Method Call `x.m()` with Auto-Addressing:**
        ...
        -   If `Point` is a struct (value type) and GoScript maps it to an object literal or a class that needs explicit varRefing for pointer receivers, `WriteCallExpr` (via `WriteSelectorExpr` logic) needs to handle this. `$.callMethod(p, "Scale", [3.5])` could manage this, taking address if needed.
...
-   **GoScript & Divergences:**
    ...
    -   **Passing Slice with `...`:**
        ...
        -   **Underlying Array:** TS spread `...s` creates a shallow copy for function arguments. If `s` is a `$.Slice` that represents a view, `...s.toArray()` might be needed, or `$.callVariadic(Greeting, "msg:", s)`.
        -   If `s` is a `$.Slice`, passing `s` directly (if `...T` becomes `$.Slice<T>`) would preserve sharing if `$.Slice` is designed for it. However, Go's spec says "value passed is a new slice ... whose successive elements are the actual arguments" unless `s...` syntax is used with an existing slice.
        -   If `p` is `...T` (so `[]T` in Go func), and we call `f(sliceVal...)`:
            ...
            -   **Divergence:** For `s...` to truly share, the `$.Slice` itself would need to be passed, and the function signature adapted, or runtime helpers used. `$.callWithSpread(f, args, s_gsSlice)` could handle this.
