...
*   **Composite Types:**
    ...
    *   **Maps:** Translated to TypeScript `Map<K, V>` through `$.makeMap`. The runtime indexes string keys by their byte values and flat structs containing only strings by their field names and byte values. Struct buckets use the shared Go comparison routine to decide equality; updates retain the original stored key. Pointer maps retain the declared key type, including generic instantiations, and index keys through the runtime pointer-alias identity. They do not inspect pointee fields. Other comparable keys use the existing comparison scan.
